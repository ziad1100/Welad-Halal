...
export function POSPage({ onBack }: { onBack: () => void }) {
  ...
  async function searchBarcode() {
    ...
    try {
      ...
      if (data) { addToCart(data); setQ(''); }
    } catch {
      // unknown barcode → open product modal prefilled (§40)
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }} dir="rtl">
      ...
      <div style={{ background: 'var(--k-detail-header)', borderBottom: '1px solid var(--k-border)', padding: '4px 8px', display: 'flex', gap: 16 }}>
        ...
        <span>العميل: <b>{cart.customerName}</b></span>
      </div>

      {/* customer area §22 */}
      <div className="krow" style={{ padding: '4px 8px' }}>
        <button className="kbtn" onClick={() => cart.setCustomer(null, 'عميل نقدي')}>عميل نقدي</button>
      ...
      <div className="krow" style={{ padding: '0 8px 4px' }}>
        ...
        <select className="kselect" value={cat} onChange={(e) => setCat(e.target.value)}>
      ...
      {showCustomers && <CustomerModal onClose={() => setShowCustomers(false)} onSelect={(c) => { cart.setCustomer(c.id, c.name); setShowCustomers(false); }} />}
      {showProduct && <ProductModal barcode={prefillBarcode} onClose={() => setShowProduct(false)} onSaved={(p) => { setShowProduct(false); setQ(''); refetchProducts(); if (p) addToCart(p); }} />}
    </div>
  );
}
