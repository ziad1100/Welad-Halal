...
export class PurchasesService {
  ...
  list() { return this.prisma.purchaseOrder.findMany({ include: { items: { include: { product: true } } }, orderBy: { createdAt: 'desc' }, take: 100 }); }
  async create(dto: { supplierName: string; notes?: string; items: { productId: string; quantity: number; purchasePrice: number }[] }, userId: string) {
    return this.prisma.$transaction(async (tx) => {
      ...
      const po = await tx.purchaseOrder.create({ data: { supplierName: dto.supplierName, notes: dto.notes, subtotal, total: subtotal, createdById: userId } });
